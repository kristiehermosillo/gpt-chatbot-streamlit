...
# Display messages and edit option for the last user message only
last_user_idx = max(
...
for i, msg in enumerate(st.session_state.messages):
    ...
    if role == "user" and st.session_state.edit_index == i:
        ...
        if st.button("↩️ Resend", key=f"resend_{i}"):
            ...
            st.session_state.messages = st.session_state.messages[:i+1]  # Cut off anything after
...
# Show input box if not editing or processing
